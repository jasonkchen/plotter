...
def load():
    ...
    for a in window.grid_slaves():
        # Delete "File Not Found" label
        if int(a.grid_info()["row"]) < 1 and int(a.grid_info()["column"]) < 3 and int(a.grid_info()["column"]) > 1:
            a.destroy()
        
        # Delete old table
        elif int(a.grid_info()["row"]) > 5:
            a.destroy()

    # Find XLSX file and load table
    try:
